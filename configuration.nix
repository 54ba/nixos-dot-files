...

{
  imports = [
    ...
    ./modules/mobile-integration.nix             # ENABLED - Part of mobile suite (conflicts)
    ...
    ./modules/device-permissions.nix            # ENABLED - Device permission management (conflicts)
  ...
  custom.services.system = {
    ...
    android.enable = true;                          # Disable Android tools for now
  ...
  custom.services.security = {
    ...
    firewall = {
      ...
      allowedTCPPorts = [ 22 80 443 25 465 587 ];   # Allow SSH, HTTP, HTTPS, SMTP
  ...
  # Development Libraries Configuration
...
# Systemd service to fix session environment
systemd.user.services.fix-session-env = {
  enable = true;
  description = "Fix session environment for GUI applications";
  wantedBy = [ "graphical-session.target" ];
  partOf = [ "graphical-session.target" ];
  serviceConfig = {
    Type = "oneshot";
    ExecStart = "/etc/nixos/scripts/fix-session-env.sh";
    RemainAfterExit = true;
  };
};

# Ensure XDG runtime directories are created
systemd.tmpfiles.rules = [
  "d /run/user/1000 0755 mahmoud users -"
  "d /run/user/1000/flatpak 0755 mahmoud users -"
];

# Fix for GNOME session and Flatpak
...
services.xserver.displayManager.sessionCommands = ''
  # Set Wayland environment for display manager
  ...
  # Ensure Flatpak session helper can run
  if [ -n "$XDG_RUNTIME_DIR" ] && [ -d "$XDG_RUNTIME_DIR" ]; then
    export FLATPAK_SESSION_HELPER="$XDG_RUNTIME_DIR/flatpak-session-helper"
  fi
'';

  # NVIDIA Performance Optimizations - ENABLED for proper GPU support
  ...
  users.users.mahmoud = {
    isNormalUser = true;
    description = "mahmoud";
    extraGroups = [ "wheel" "networkmanager" "video" "audio" "docker" "libvirtd" "kvm" "input" "systemd-journal" "adbusers" "plugdev" "fuse" ];
...
EOF
  ...
  custom.mobile-integration = {
    enable = true;                            # Enable mobile device integration
    
    # KDE Connect for comprehensive mobile integration
    kdeConnect = {
      enable = true;                          # Enable KDE Connect
      features = {
        notifications = true;                 # Enable notification sync
        clipboard = true;                     # Enable clipboard sharing
        fileTransfer = true;                  # Enable file transfer
        mediaControl = true;                  # Enable media control
        remoteInput = true;                   # Enable remote input
        sms = true;                           # Enable SMS integration
        findDevice = true;                    # Enable find device
      };
    };
    
    # Android device integration
    android = {
      enable = true;                          # Enable Android support
      scrcpy = {
        enable = true;                        # Enable scrcpy for screen mirroring
        quality = 8;                          # Best video quality
        wireless = true;                      # Enable wireless control
        recordScreen = true;                  # Enable screen recording
      };
      adb = {
        enable = true;                        # Enable ADB
        wireless = true;                      # Enable wireless ADB
        port = 5555;                          # ADB wireless port
      };
      apps = {
        enable = true;                        # Enable Android app management
        apkTool = false;                      # Disable APK analysis tools
      };
    };
    
    # iOS device integration
    ios = {
      enable = false;                         # Disable iOS support
      libimobiledevice = false;               # Disable libimobiledevice
      uxplay = false;                         # Disable AirPlay
      fileTransfer = false;                   # Disable iOS file transfer
    };
    
    # Cross-platform features
    crossPlatform = {
      enable = true;                          # Enable cross-platform features
      localsend = true;                       # Enable LocalSend
      warpinator = true;                      # Enable Warpinator
      snapdrop = false;                       # Disable Snapdrop
      pairdrop = true;                        # Enable PairDrop
    };
    
    # Web interface for mobile control
    webInterface = {
      enable = false;                          # Disable web interface (port conflict)
      port = 8081;                            # Web interface port
      ssl = false;                            # Disable SSL for local use
      authentication = true;                  # Enable authentication
    };
    
    # Notifications
    notifications = {
      enable = true;                          # Enable notification forwarding
      pushover = false;                       # Disable Pushover
      ntfy = true;                            # Enable ntfy.sh
      telegram = false;                       # Disable Telegram
    };
    
    # Automation features
    automation = {
      enable = true;                          # Enable automation
      homeAssistant = false;                  # Disable Home Assistant
      shortcuts = true;                       # Enable mobile shortcuts
      tasker = true;                          # Enable Tasker integration
    };
  };
